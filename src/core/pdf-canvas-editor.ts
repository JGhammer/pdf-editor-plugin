...
export class PDFCanvasEditor {
  ...
  constructor(container: HTMLElement, scale = 1.5, cb: CanvasEditorCallbacks = {}) {
...
.pdf-ce{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif}
...
.pdf-ce-ta{position:absolute;border:2px solid #00d9ff;background:rgba(255,255,255,.95);color:#000;font-size:inherit;font-family:inherit;padding:2px 4px;resize:none;outline:none;z-index:20;overflow:hidden;line-height:1.4}
...
`;
  ...
  private async renderAll() {
    ...
    for (let i = 0; i < this.totalPages; i++) {
      ...
      ov.addEventListener("mousedown", (e) => { if (e.target === ov) this.deselect(); this.currentPage = i; this.updateNav(); });
      ov.addEventListener("dblclick", (e) => { if (e.target === ov) this.addTextAt(i, e.offsetX, e.offsetY); });
  ...
  private buildNav(): HTMLDivElement {
    ...
    prev.onclick = () => { if (this.currentPage > 0) { this.currentPage--; this.scrollPage(); this.updateNav(); } };
    ...
    next.onclick = () => { if (this.currentPage < this.totalPages - 1) { this.currentPage++; this.scrollPage(); this.updateNav(); } };
  ...
  private updateNav() {
    ...
    if (btns[0]) (btns[0] as HTMLButtonElement).disabled = this.currentPage <= 0;
    if (btns[1]) (btns[1] as HTMLButtonElement).disabled = this.currentPage >= this.totalPages - 1;
  }

  private scrollPage() {
    this.pageWrappers[this.currentPage]?.scrollIntoView({ behavior: "smooth", block: "start" });
  }

  private addText() {
    const ov = this.overlays[this.currentPage];
    if (!ov) return;
    this.addTextAt(this.currentPage, ov.clientWidth * 0.1, ov.clientHeight * 0.1);
  }

  private addTextAt(pi: number, x: number, y: number) {
    const el: EditorElement = { id: this.uid(), type: "text", pageIndex: pi, x, y, width: 200, height: 30, content: "请输入文字", fontSize: 14, fontFamily: "sans-serif", color: "#000000", bgColor: "transparent", opacity: 1 };
  ...
  private addImage() {
    ...
    input.onchange = async () => {
      ...
      const el: EditorElement = { id: this.uid(), type: "image", pageIndex: this.currentPage, x: 50, y: 50, width: 200, height: 150, content: "", fontSize: 14, fontFamily: "sans-serif", color: "#000", bgColor: "transparent", opacity: 1, dataUrl: url };
      this.elements.push(el);
      this.renderEls();
      this.select(el.id);
      this.cb.onElementsChange?.(this.elements);
  ...
  private addRect() {
    const el: EditorElement = { id: this.uid(), type: "rect", pageIndex: this.currentPage, x: 50, y: 50, width: 150, height: 40, content: "", fontSize: 14, fontFamily: "sans-serif", color: "#000", bgColor: "#ffffff", opacity: 1 };
    this.elements.push(el);
    this.renderEls();
    this.select(el.id);
    this.cb.onElementsChange?.(this.elements);
  ...
  private select(id: string) {
    this.deselect();
    this.selectedId = id;
    const dom = this.container.querySelector(`[data-eid="${id}"]`) as HTMLElement;
    if (dom) {
  ...
  private deselect() {
    this.selectedId = null;
    this.finishEdit();
    this.container.querySelectorAll(".pdf-ce-el.sel").forEach((d) => { d.classList.remove("sel"); d.querySelectorAll(".pdf-ce-rh,.pdf-ce-del").forEach((h) => h.remove()); });
  }
  ...
  private addHandles(dom: HTMLElement, id: string) {
    ...
    for (const p of ["nw", "ne", "sw", "se"]) {
      ...
      h.addEventListener("mousedown", (e) => { e.stopPropagation(); this.startResize(e, id, p); });
      dom.appendChild(h);
    }
    ...
    del.addEventListener("click", (e) => { e.stopPropagation(); this.deleteSelected(); });
    dom.appendChild(del);
  }

  private renderEls() {
    this.overlays.forEach((ov) => ov.querySelectorAll(".pdf-ce-el").forEach((d) => d.remove()));
    for (const el of this.elements) {
      ...
      dom.addEventListener("mousedown", (e) => { e.stopPropagation(); this.select(el.id); this.startDrag(e, el.id); });
      dom.addEventListener("dblclick", (e) => { e.stopPropagation(); if (el.type === "text") { this.select(el.id); this.startEdit(el); } });
  ...
  private startDrag(e: MouseEvent, id: string) {
    ...
    const move = (ev: MouseEvent) => {
      ...
      const d = this.container.querySelector(`[data-eid="${id}"]`) as HTMLElement;
      if (d) { d.style.left = el.x + "px"; d.style.top = el.y + "px"; }
    };
    const up = () => { this.dragging = false; document.removeEventListener("mousemove", move); document.removeEventListener("mouseup", up); this.renderEls(); this.select(id); this.cb.onElementsChange?.(this.elements); };
  ...
  private startResize(e: MouseEvent, id: string, dir: string) {
    ...
    const move = (ev: MouseEvent) => {
      ...
      if (this.resizeDir.includes("w")) { const nw = Math.max(10, this.sEw - dx); el.x = this.sEx + this.sEw - nw; el.width = nw; }
      if (this.resizeDir.includes("s")) el.height = Math.max(10, this.sEh + dy);
      if (this.resizeDir.includes("n")) { const nh = Math.max(10, this.sEh - dy); el.y = this.sEy + this.sEh - nh; el.height = nh; }
      const d = this.container.querySelector(`[data-eid="${id}"]`) as HTMLElement;
      if (d) { d.style.left = el.x + "px"; d.style.top = el.y + "px"; d.style.width = el.width + "px"; d.style.height = el.height + "px"; }
    };
    const up = () => { this.resizing = false; document.removeEventListener("mousemove", move); document.removeEventListener("mouseup", up); this.renderEls(); this.select(id); this.cb.onElementsChange?.(this.elements); };
    document.addEventListener("mousemove", move);
    document.addEventListener("mouseup", up);
  }

  private startEdit(el: EditorElement) {
    this.finishEdit();
    const ov = this.overlays[el.pageIndex];
    if (!ov) return;
    const ta = document.createElement("textarea");
    ta.className = "pdf-ce-ta";
    ta.style.cssText = `left:${el.x}px;top:${el.y}px;width:${el.width}px;height:${el.height}px;font-size:${el.fontSize}px;font-family:${el.fontFamily};color:${el.color}`;
    ta.value = el.content;
    ov.appendChild(ta);
    ta.focus();
    ta.select();
    this.textarea = ta;
    ta.addEventListener("blur", () => { el.content = ta.value; this.finishEdit(); this.renderEls(); this.select(el.id); this.cb.onElementsChange?.(this.elements); });
  ...
  getElements(): EditorElement[] { return [...this.elements]; }

  getSelectedElement(): EditorElement | null { return this.elements.find((e) => e.id === this.selectedId) || null; }
  ...
  async exportPDF(): Promise<Uint8Array> {
    ...
    for (const el of this.elements) {
      ...
      if (el.type === "rect") {
        ...
        pg.drawRectangle({ x: px, y: py, width: pw, height: el.height * inv, color: rgb(r, g, b), opacity: el.opacity });
        ...
        el.content.split("\n").forEach((line, i) => {
          try { pg.drawText(line.replace(/[^\x20-\x7E]/g, "?"), { x: px + 2 * inv, y: py + el.height * inv - (i + 1) * pfs * 1.4, size: pfs, font, color: rgb(r, g, b), opacity: el.opacity }); } catch {}
        });
      } else if (el.type === "image" && el.dataUrl) {
        try {
          const ib = this.dataURLtoBytes(el.dataUrl);
          const img = el.dataUrl.includes("image/png") ? await doc.embedPng(ib) : await doc.embedJpg(ib);
          pg.drawImage(img, { x: px, y: py, width: pw, height: el.height * inv, opacity: el.opacity });
        } catch {}
      }
    }
    return doc.save();
  ...
  private uid(): string { return "e" + Date.now() + Math.random().toString(36).slice(2, 7); }
  private readDataURL(f: File): Promise<string> { return new Promise((r) => { const rd = new FileReader(); rd.onload = (e) => r(e.target!.result as string); rd.readAsDataURL(f); }); }
  private dataURLtoBytes(u: string): Uint8Array { const b = atob(u.split(",")[1]); const a = new Uint8Array(b.length); for (let i = 0; i < b.length; i++) a[i] = b.charCodeAt(i); return a; }
